...
namespace _02._Ad_Astra
{
    ...
    //
    //Calculate the total calories of all food items and then determine how many days you can last with the food you have. Keep in mind that you need 2000 kcal a day.
    //
    //Input / Constraints
    ...
    //  The sum of their calories is 4700. Since you need 2000 kcal a day, we divide 4700/2000, which means this food will last you for 2 days.
    //  We print each item.
    //Input
    ...
    //  "Invalid food" is not a match since it doesn't have a # at the end.
    ...
    class Program
    {
        static void Main()
        {
            ...
            string foodPattern = @"(?<splitter>#|\|)(?<itemName>[a-zA-Z ]+)\k<splitter>(?<expirationDate>\d{2}/\d{2}/\d{2})\k<splitter>(?<calories>\d{1,5})\k<splitter>";
            MatchCollection productMatches = Regex.Matches(input, foodPattern);
            ...
            foreach (Match product in productMatches)
            {
                Console.WriteLine($"Item: {product.Groups["itemName"].Value}, Best before: " +
                    $"{product.Groups["expirationDate"].Value}, Nutrition: {product.Groups["calories"].Value}");
            }
        }
    }
}
